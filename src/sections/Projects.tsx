          ...
          {portfolioProjects.map((project) => (
            <div
              ...
              className="bg-gray-800 rounded-3xl relative z-0 overflow-hidden after:z-10 after:content-[''] after:absolute after:inset-0 after:outline-2 after:outline after:-outline-offset-2 after:rounded-3xl after:outline-white/20 px-8 pt-8 md:pt-12 md:px-10 after:pointer-events-none">
                ...
                ></div> 
                <div className="bg-gradient-to-r from-emerald-300 to-sky-400 inline-fex gap-2 font-bold uppercase tracking-widest text-sm text-transparent bg-clip-text">
                <span>{project.company}</span>
                <span> &bull; </span>
                <span>{project.year}</span>
              </div>
              <h3 className="font-serif font-semibold text-2xl mt-2 md:mt-5 md:text-4xl">{project.title}</h3>
              ...
              <a href={project.link}>
                <button className="bg-white text-gray-950 h-12 px-6 w-full md:w-auto px-4 rounded-xl font-semibold inline-flex items-center justify-center gap-2 mt-8">
